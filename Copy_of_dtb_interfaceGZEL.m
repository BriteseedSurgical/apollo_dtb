function varargout = dtb_interfaceGZEL(varargin)
...
for i = 1:num_rows
    ...
    tab_text{i} = string;
    new_command = sprintf('M X%.2f Y%.2f',x_pos(end),y_pos(end));
    commands = [commands,new_command];
    row = row+1;
end
...
% Initialize the SERIAL port for the DTB
handles.dtb = serial("/dev/ttyACM0",'BaudRate',115200);
stat = handles.dtb.Status;
if strcmpi(stat,'open')
    flose(handles.dtb);
    fopen(handles.dtb);
else
    fopen(handles.dtb);
...
switch selection
    case contents{1} % Move to
        coords = inputdlg({'X (mm)', 'Y (mm)', 'Z (mm)', 'Speed (mm/s)'}, 'Move to...',[1,15],{'0','0','0','270'});%% Giselle Addition
        try
...
for x = 1:length(commands)
fwrite(handles.dtb,commands(x));
end
...
if strcmpi(received,'ok')
    ...
    printToFeedbackBox(handles, strcat("MOVE Status: in progress/ done"));
else
    printToFeedbackBox(handles, "Error: Motor Not Connected") 
    printToFeedbackBox(handles, strcat("Response: not moving")) 
end
...
y = coords(2)-handles.step;
fprintf(handles.dtb, "M Y %.2f",y);
coords(2) = y;
...
if strcmpi(received,'ok')
    printToFeedbackBox(handles, "Success, Motor Connected.") ;
    printToFeedbackBox(handles, strcat("MOVE Status: done"));
else
    printToFeedbackBox(handles, "Error: Motor Not Connected") 
    printToFeedbackBox(handles, strcat("Response: moved")) 
end
...
if strcmpi(received,'ok')
    ...
    printToFeedbackBox(handles, "Error: Motor Not Connected") 
    printToFeedbackBox(handles, strcat("Response: NOT HOME")) 
end
...
coords = handles.coords;
x = coords(1)-handles.step;
fprintf(handles.dtb, "M X %.2f",x);
coords(1) = x;
handles.coords = coords;
received = fgets(handles.MotorSerial);    
%can receive one of two valid responses
